...
class CodeService {
	async getQnaCommentList(json?:any) { // QNA 댓글 리스트 [CS_QNA_CMMT]
		const res = await jsonApi('/bo/cs/getQnaCommentList', JSON.parse(json));
		return res.data;
	}
	async getQnaComment(json?:any) { // QNA 댓글 상세조회 [CS_QNA_CMMT]
		const res = await jsonApi('/bo/cs/getQnaComment', JSON.parse(json));
		return res.data;
	}
	async setQnaCommentList(json?:any) { // QNA 댓글 리스트등록 [CS_QNA_CMMT]
		const res = await jsonApi('/bo/cs/setQnaCommentList', JSON.parse(json));
		return res.data;
	}
	async insertQnaComment(json?:any) { // QNA 댓글 등록 [CS_QNA_CMMT]
		const res = await jsonApi('/bo/cs/insertQnaComment', JSON.parse(json));
		return res.data;
	}
	async updateQnaComment(json?:any) { // QNA 댓글 수정 [CS_QNA_CMMT]
		const res = await jsonApi('/bo/cs/updateQnaComment', JSON.parse(json));
		return res.data;
	}
