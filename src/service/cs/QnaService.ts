...
class CodeService {
	async getQnaList(json?:any) { // QNA 리스트 [CS_QNA]
		const res = await jsonApi('/bo/cs/getQnaList', json);
		return res.data;
	}
	async getQna(json?:any) { // QNA 상세조회 [CS_QNA]
		const res = await jsonApi('/bo/cs/getQna', json);
		return res.data;
	}
	async setQnaList(json?:any) { // QNA 리스트등록 [CS_QNA]
		const res = await jsonApi('/bo/cs/setQnaList', json);
		return res.data;
	}
	async insertQna(json?:any) { // QNA 등록 [CS_QNA]
		const res = await jsonApi('/bo/cs/insertQna', json);
		return res.data;
	}
	async updateQna(json?:any) { // QNA 수정 [CS_QNA]
		const res = await jsonApi('/bo/cs/updateQna', json);
		return res.data;
	}
