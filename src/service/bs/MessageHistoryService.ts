...
class CodeService {
	async getMessageHistoryList(json?:any) { // 문자메시지이력 리스트 [BS_MSG_HIST]
		const res = await jsonApi('/bo/bs/getMessageHistoryList', JSON.parse(json));
		return res.data;
	}
	async getMessageHistory(json?:any) { // 문자메시지이력 상세조회 [BS_MSG_HIST]
		const res = await jsonApi('/bo/bs/getMessageHistory', JSON.parse(json));
		return res.data;
	}
	async setMessageHistoryList(json?:any) { // 문자메시지이력 리스트등록 [BS_MSG_HIST]
		const res = await jsonApi('/bo/bs/setMessageHistoryList', JSON.parse(json));
		return res.data;
	}
	async insertMessageHistory(json?:any) { // 문자메시지이력 등록 [BS_MSG_HIST]
		const res = await jsonApi('/bo/bs/insertMessageHistory', JSON.parse(json));
		return res.data;
	}
	async updateMessageHistory(json?:any) { // 문자메시지이력 수정 [BS_MSG_HIST]
		const res = await jsonApi('/bo/bs/updateMessageHistory', JSON.parse(json));
		return res.data;
	}
