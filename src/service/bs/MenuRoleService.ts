...
class CodeService {
	async getMenuRoleList(json?:any) { // 업체메뉴권한 리스트 [BS_MENU_ROLE]
		const res = await jsonApi('/bo/bs/getMenuRoleList', JSON.parse(json));
		return res.data;
	}
	async getMenuRole(json?:any) { // 업체메뉴권한 상세조회 [BS_MENU_ROLE]
		const res = await jsonApi('/bo/bs/getMenuRole', JSON.parse(json));
		return res.data;
	}
	async setMenuRoleList(json?:any) { // 업체메뉴권한 리스트등록 [BS_MENU_ROLE]
		const res = await jsonApi('/bo/bs/setMenuRoleList', JSON.parse(json));
		return res.data;
	}
	async insertMenuRole(json?:any) { // 업체메뉴권한 등록 [BS_MENU_ROLE]
		const res = await jsonApi('/bo/bs/insertMenuRole', JSON.parse(json));
		return res.data;
	}
	async updateMenuRole(json?:any) { // 업체메뉴권한 수정 [BS_MENU_ROLE]
		const res = await jsonApi('/bo/bs/updateMenuRole', JSON.parse(json));
		return res.data;
	}
