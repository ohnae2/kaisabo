...
class CodeService {
	async getLinkHistoryList(json?:any) { // 연동이력 리스트 [BS_LINK_HIST]
		const res = await jsonApi('/bo/bs/getLinkHistoryList', JSON.parse(json));
		return res.data;
	}
	async getLinkHistory(json?:any) { // 연동이력 상세조회 [BS_LINK_HIST]
		const res = await jsonApi('/bo/bs/getLinkHistory', JSON.parse(json));
		return res.data;
	}
	async setLinkHistoryList(json?:any) { // 연동이력 리스트등록 [BS_LINK_HIST]
		const res = await jsonApi('/bo/bs/setLinkHistoryList', JSON.parse(json));
		return res.data;
	}
	async insertLinkHistory(json?:any) { // 연동이력 등록 [BS_LINK_HIST]
		const res = await jsonApi('/bo/bs/insertLinkHistory', JSON.parse(json));
		return res.data;
	}
	async updateLinkHistory(json?:any) { // 연동이력 수정 [BS_LINK_HIST]
		const res = await jsonApi('/bo/bs/updateLinkHistory', JSON.parse(json));
		return res.data;
	}
