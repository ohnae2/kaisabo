...
class CodeService {
	async getApiHistoryList(json?:any) { // API이력 리스트 [BS_API_HIST]
		const res = await jsonApi('/bo/bs/getApiHistoryList', json);
		return res.data;
	}
	async getApiHistory(json?:any) { // API이력 상세조회 [BS_API_HIST]
		const res = await jsonApi('/bo/bs/getApiHistory', json);
		return res.data;
	}
	async setApiHistoryList(json?:any) { // API이력 리스트등록 [BS_API_HIST]
		const res = await jsonApi('/bo/bs/setApiHistoryList', json);
		return res.data;
	}
	async insertApiHistory(json?:any) { // API이력 등록 [BS_API_HIST]
		const res = await jsonApi('/bo/bs/insertApiHistory', json);
		return res.data;
	}
	async updateApiHistory(json?:any) { // API이력 수정 [BS_API_HIST]
		const res = await jsonApi('/bo/bs/updateApiHistory', json);
		return res.data;
	}
