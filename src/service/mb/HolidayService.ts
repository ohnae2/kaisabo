...
class CodeService {
	async getHolidayList(json?:any) { // 휴일 리스트 [MB_HLD]
		const res = await jsonApi('/bo/mb/getHolidayList', json);
		return res.data;
	}
	async getHoliday(json?:any) { // 휴일 상세조회 [MB_HLD]
		const res = await jsonApi('/bo/mb/getHoliday', json);
		return res.data;
	}
	async setHolidayList(json?:any) { // 휴일 리스트등록 [MB_HLD]
		const res = await jsonApi('/bo/mb/setHolidayList', json);
		return res.data;
	}
	async insertHoliday(json?:any) { // 휴일 등록 [MB_HLD]
		const res = await jsonApi('/bo/mb/insertHoliday', json);
		return res.data;
	}
	async updateHoliday(json?:any) { // 휴일 수정 [MB_HLD]
		const res = await jsonApi('/bo/mb/updateHoliday', json);
		return res.data;
	}
