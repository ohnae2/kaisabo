<template>
	<form class="search" @submit.prevent="getList" @keyup.enter="getList">
		...
		<div class="btnWrap">
			<span class="crud">
				<button type="button" class="button add" @click="gridUtil.add(data.orderGrid, 0)"><span class="icon">&#xe813;</span>추가</button>
				<button type="button" class="button save" @click="save"><span class="icon">&#xe814;</span>저장</button>
				<button type="button" class="button del" @click="gridUtil.del(data.orderGrid)"><span class="icon">&#xe815;</span>삭제</button>
			</span>
...
const data = reactive({
	orderGrid: {} as Grid,
	required: ['prodNo', 'ordStatCd', 'mbrNm', 'telNo', 'rsvDay', 'email', 'price'],
	totalCount: 0,
	list: [],
	audit: false,
});
const getList = () => {
	OrderService.getOrderList(search).then(
		(res) => {
			...
			data.list = res.data;
			data.orderGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const save = () => {
	let saveList = gridUtil.save(data.orderGrid, data.required);
	if(saveList) {
...
onMounted(() => {
	data.orderGrid = new Grid({
		el: document.getElementById('orderGrid') as HTMLElement,
		// rowHeaders: ['checkbox'],
		columns: [
