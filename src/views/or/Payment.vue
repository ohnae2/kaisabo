...
const data = reactive({
	paymentGrid: {} as Grid,
	required: ['ordNo', 'prodNo', 'payPrice', 'payWayCd', 'payDt'],
	totalCount: 0,
	list: [],
	audit: false,
});
const getList = () => {
	PaymentService.getPaymentList(search).then(
		(res) => {
			...
			data.list = res.data;
			data.paymentGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.paymentGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.paymentGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.paymentGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	data.paymentGrid.blur();
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.paymentGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.paymentGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.paymentGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
	...
	if(confirm('등록 ' + count[0] + '건, 수정 ' + count[1] + '건, 삭제 ' + count[2] + '건을 정말 저장하시겠습니까?')) {
		PaymentService.setPaymentList(saveList).then(
			(res) => {
				location.reload();
			},
			(err) => {
...
onMounted(() => {
	data.paymentGrid = new Grid({
		...
		columns: [
			...
			{header: '결제요금', name: 'payPrice', sortable: true, width: 100, align: 'right', disabled: false, validation: { dataType: 'number' , required: false }, editor: 'text'}, // 결제요금
			{header: '결제수단코드', name: 'payWayCd', width: 120, align: 'left', sortable: true, disabled: false, validation: { dataType: 'string' , required: false }, 
			...
			{header: '가맹점코드', name: 'mcnsCd', width: 120, align: 'left', sortable: true, disabled: false, validation: { dataType: 'string' , required: true }, 
				formatter: 'listItemText',
				editor: {
					type: 'select',
					options: {
			...
			{header: '결제일시', name: 'payDt', sortable: true, width: 120, align: 'left', disabled: false, validation: { dataType: 'string' , required: false }, // 결제일시
				editor: {
				type: 'datePicker',
					options: {
	...
	data.paymentGrid.on('click', (e:any) => {
		if( e.columnName === 'cd') {
			console.log('click')
		}
	});
