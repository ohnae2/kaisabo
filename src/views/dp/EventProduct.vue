...
// 이벤트상품
const data = reactive({
	eventProductGrid: {} as Grid,
	required: ['evtNo', 'prodNo', 'strtDt', 'endDt'],
	totalCount: 0,
	list: [],
	audit: false,
});
const getList = () => {
	EventProductService.getEventProductList(search).then(
		(res) => {
			data.totalCount = (res.count) ? res.count : 0;
			data.list = res.data;
			data.eventProductGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.eventProductGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.eventProductGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.eventProductGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	data.eventProductGrid.blur();
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.eventProductGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.eventProductGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.eventProductGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
...
onMounted(() => {
	data.eventProductGrid = new Grid({
		el: document.getElementById('eventProductGrid') as HTMLElement,
		//rowHeaders: ['checkbox'],
		columns: [
			{header: '이벤트번호', name: 'evtNo', sortable: true, width: 100, align: 'right', disabled: false, validation: { dataType: 'number' , required: false }, editor: 'text'}, // 이벤트번호
	...
	data.eventProductGrid.on('click', (e:any) => {
		if( e.columnName === 'cd') {
			console.log('click')
		}
	});
