...
const getList = () => {
	BoardCommentService.getBoardCommentList(search).then(
		(res) => {
			data.totalCount = (res.count) ? res.count : 0;
			data.list = res.data;
			data.boardCommentGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.boardCommentGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.boardCommentGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.boardCommentGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	data.boardCommentGrid.blur();
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.boardCommentGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.boardCommentGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.boardCommentGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
	...
	if(confirm('등록 ' + count[0] + '건, 수정 ' + count[1] + '건, 삭제 ' + count[2] + '건을 정말 저장하시겠습니까?')) {
		BoardCommentService.setBoardCommentList(saveList).then(
			(res) => {
				location.reload();
			},
			(err) => {
...
onMounted(() => {
	data.boardCommentGrid = new Grid({
		el: document.getElementById('boardCommentGrid') as HTMLElement,
		//rowHeaders: ['checkbox'],
		columns: [
			{header: '게시판댓글번호', name: 'brdCmmtNo', sortable: true, width: 100, align: 'right', disabled: true, validation: { dataType: 'number' , required: false }, editor: 'text'}, // 게시판댓글번호
			{header: '게시판번호', name: 'brdNo', sortable: true, width: 100, align: 'right', disabled: false, validation: { dataType: 'number' , required: false }, editor: 'text'}, // 게시판번호
	...
	data.boardCommentGrid.on('click', (e:any) => {
		if( e.columnName === 'cd') {
			console.log('click')
		}
	});
