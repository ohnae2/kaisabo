...
const data = reactive({
	displayGrid: {} as Grid,
	required: ['cmpDivCd', 'dispCd', 'prir', 'dispYn'],
	totalCount: 0,
	list: [],
	audit: false,
});
const getList = () => {
	DisplayService.getDisplayList(search).then(
		(res) => {
			...
			data.list = res.data;
			data.displayGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.displayGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.displayGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.displayGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	data.displayGrid.blur();
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.displayGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.displayGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.displayGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
	...
	if(confirm('등록 ' + count[0] + '건, 수정 ' + count[1] + '건, 삭제 ' + count[2] + '건을 정말 저장하시겠습니까?')) {
		DisplayService.setDisplayList(saveList).then(
			(res) => {
				location.reload();
			},
			(err) => {
...
onMounted(() => {
	data.displayGrid = new Grid({
		...
		columns: [
			...
			{header: '전시코드', name: 'dispCd', width: 120, align: 'left', sortable: true, disabled: false, validation: { dataType: 'string' , required: false }, 
				formatter: 'listItemText',
				editor: {
					type: 'select',
					options: {
	...
	data.displayGrid.on('click', (e:any) => {
		if( e.columnName === 'cd') {
			console.log('click')
		}
	});
