...
const getList = () => {
	HolidayService.getHolidayList(search).then(
		(res) => {
			...
			data.list = res.data;
			data.holidayGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.holidayGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.holidayGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.holidayGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	data.holidayGrid.blur();
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.holidayGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.holidayGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.holidayGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
	...
	if(confirm('등록 ' + count[0] + '건, 수정 ' + count[1] + '건, 삭제 ' + count[2] + '건을 정말 저장하시겠습니까?')) {
		HolidayService.setHolidayList(saveList).then(
			(res) => {
				location.reload();
			},
			(err) => {
...
onMounted(() => {
	...
	data.holidayGrid.on('click', (e:any) => {
		if( e.columnName === 'cd') {
			console.log('click')
		}
	});
