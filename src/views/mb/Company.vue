...
const getList = () => {
	CompanyService.getCompanyList(search).then(
		(res) => {
			...
			data.list = res.data;
			data.companyGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.companyGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.companyGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.companyGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	data.companyGrid.blur();
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.companyGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.companyGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.companyGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
