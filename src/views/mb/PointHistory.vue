...
const getList = () => {
	PointHistoryService.getPointHistoryList(search).then(
		(res) => {
			...
			data.list = res.data;
			data.pointHistoryGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.pointHistoryGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.pointHistoryGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.pointHistoryGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	data.pointHistoryGrid.blur();
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.pointHistoryGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.pointHistoryGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.pointHistoryGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
...
onMounted(() => {
	...
	data.pointHistoryGrid.on('click', (e:any) => {
		if( e.columnName === 'cd') {
			console.log('click')
		}
	});
