...
const data = reactive({
	pointGrid: {} as Grid,
	required: ['pntNm', 'pntPrice', 'strtDt', 'endDt'],
	totalCount: 0,
	list: [],
	audit: false,
});
const getList = () => {
	PointService.getPointList(search).then(
		(res) => {
			data.totalCount = (res.count) ? res.count : 0;
			data.list = res.data;
			data.pointGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.pointGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.pointGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.pointGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	...
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.pointGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.pointGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.pointGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
	...
	if(confirm('등록 ' + count[0] + '건, 수정 ' + count[1] + '건, 삭제 ' + count[2] + '건을 정말 저장하시겠습니까?')) {
		PointService.setPointList(saveList).then(
			(res) => {
				location.reload();
			},
			(err) => {
...
onMounted(() => {
	data.pointGrid = new Grid({
		el: document.getElementById('pointGrid') as HTMLElement,
		//rowHeaders: ['checkbox'],
		columns: [
