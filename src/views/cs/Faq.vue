...
const add = () => {
	data.faqGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.faqGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.faqGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	...
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.faqGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.faqGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.faqGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
	...
	if(confirm('등록 ' + count[0] + '건, 수정 ' + count[1] + '건, 삭제 ' + count[2] + '건을 정말 저장하시겠습니까?')) {
		FaqService.setFaqList(saveList).then(
			(res) => {
				location.reload();
			},
			(err) => {
