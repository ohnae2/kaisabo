...
const data = reactive({
	grid: {} as Grid,
	required: ['usrId', 'mbrId', 'cnts', 'linkRef', 'modId', 'modDt', 'regId', 'regDt'],
	totalCount: 0,
	list: [],
	audit: false,
});
...
onMounted(() => {
	data.grid = new Grid({
		...
		//rowHeaders: ['checkbox'],
		columns: [
			{header: '메시지번호', name: 'msgNo', sortable: true, width: 100, align: 'right', disabled: false, validation: { dataType: 'number' , required: false }, editor: 'text'}, // 메시지번호
			{header: '사용자ID', name: 'usrId', sortable: true, width: 100, align: 'left', disabled: true, validation: { dataType: 'string' , required: true }, editor: 'text'}, // 사용자ID
