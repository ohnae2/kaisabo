...
// API이력
const data = reactive({
	apiHistoryGrid: {} as Grid,
	required: ['reqUrl', 'reqVal', 'rsltCd'],
	totalCount: 0,
	list: [],
	audit: false,
});
const getList = () => {
	ApiHistoryService.getApiHistoryList(search).then(
		(res) => {
			data.totalCount = (res.count) ? res.count : 0;
			data.list = res.data;
			data.apiHistoryGrid.resetData(res.data, {});
		},
		(err) => {
			console.log(err);
		},
...
const add = () => {
	data.apiHistoryGrid.appendRow({}, {at: 0});
}
const del = () => {
	let selectRow = data.apiHistoryGrid.getFocusedCell();
	if(selectRow.rowKey == null || selectRow.rowKey == undefined) {
		alert('행을 먼저 선택해주세요.');
		return;
	}
	if (confirm('선택한 행을 정말 삭제하시겠습니까?')) {
		data.apiHistoryGrid.removeRow(selectRow.rowKey);
	}
}
const refresh = () => {
	location.reload();
...
const save = () => {
	data.apiHistoryGrid.blur();
	let saveList = [];
	let count = [0, 0, 0];
	for(let o of data.apiHistoryGrid.getModifiedRows().createdRows as any) {
		o.crud = 'C';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.apiHistoryGrid.getModifiedRows().updatedRows as any) {
		o.crud = 'U';
		if(!valid(o)) {
			return;
		}
	...
	for(let o of data.apiHistoryGrid.getModifiedRows().deletedRows as any) {
		o.crud = 'D';
		saveList.push(o);
		count[2]++;
	}
...
onMounted(() => {
	data.apiHistoryGrid = new Grid({
		...
		columns: [
			...
			{header: '요청URL', name: 'reqUrl', sortable: true, width: 100, align: 'left', disabled: false, validation: { dataType: 'string' , required: false }, editor: 'text'}, // 요청URL
			{header: '요청값', name: 'reqVal', sortable: true, width: 100, align: 'left', disabled: false, validation: { dataType: 'string' , required: false }, editor: 'text'}, // 요청값
			{header: '응답값', name: 'resVal', sortable: true, width: 100, align: 'left', disabled: false, validation: { dataType: 'string' , required: true }, editor: 'text'}, // 응답값
			{header: '결과코드', name: 'rsltCd', width: 120, align: 'left', sortable: true, disabled: false, validation: { dataType: 'string' , required: false }, 
			...
			{header: 'API참조', name: 'apiRef', sortable: true, width: 100, align: 'left', disabled: false, validation: { dataType: 'string' , required: true }, editor: 'text'}, // API참조
	...
	data.apiHistoryGrid.on('click', (e:any) => {
		if( e.columnName === 'cd') {
			console.log('click')
		}
	});
